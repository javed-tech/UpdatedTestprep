import React from "react";
import { NavLink } from "react-router-dom";
import { Button } from "@material-ui/core";
import Courses from '../cardA/CardA'
export default (props) => {
      <div className="container-fluid bg-success p-4">
        <div
          className="container EducationArticles bg-light"
          style={{
            width: "80%",
            paddingTop: "0rem",
            paddingLeft: "1rem",
            paddingRight: "1rem",
            paddingBottom: "1rem",
          }}
        >
          <div className="row  m-0 p-0 ">
            <ul className="c-dmenuItems blogUpdateList">
              <li>
                <NavLink className="text-dec" to="" >
                  <h5 className="c-dHeading">COURSES IN INDIA</h5>
                </NavLink>
              </li>

              <li className="">
                <NavLink className="text-dec" to="" >
                  <h5 className="c-dHeading1">DEGREES IN INDIA</h5>
                </NavLink>
              </li>
              <hr className="m-0 p-0" width="1040px" />
            </ul>
            <div className="row">
              <div className="col-12">
                <div className=" EnteranceItems">
                  {/* Row 1 */}
                  <div className="row">
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                  </div>
                  {/* Row 2 */}
                  <div className="row mt-3">
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                  </div>
                  {/* Row 3 */}
                  <div className="row mt-3">
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                  </div>

                  {/* Row 4 */}
                  <div className="row mt-3">
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                  </div>
                  {/* Row 5 */}
                  <div className="row mt-3">
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                  </div>

                  {/* Row 6 */}
                  <div className="row mt-3">
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                  </div>

                  {/* nextRow */}
                  <div className="row">
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-1 Enterance-List-items mr pt-2 mt-3 mt-3 ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content ">
                          <h5 class="card-title text-dark">Schools</h5>
                        </div>
                      </a>
                    </div>
                    <div className="col-2  mr  mt-3   ">
                      <a href="/school/" class="text-dec ">
                        <div className="item-content  ">
                          <Button
                            variant="contained"
                            color="Secondary"
                            size="Large"
                          >
                            VIEW All
                          </Button>
                        </div>
                      </a>
                    </div>
                  </div>
                  <br />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
};

